...
nuke:
	...
	@if [ -n "$$(docker images -q $$($(DOCKER_COMPOSE) -f $(DOCKER_COMPOSE_FILE) config | grep 'image:' | awk '{print $$2}'))" ]; then docker rmi -f $$(docker images -q $$($(DOCKER_COMPOSE) -f $(DOCKER_COMPOSE_FILE) config | grep 'image:' | awk '{print $$2}')); fi
